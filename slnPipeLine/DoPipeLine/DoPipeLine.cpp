...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  //Verify2 argv[1]="process0<streamIn 2>streamErr|process1>>streamOut"
  wprintf(L"argv[1]=:%s\n",argv[1]);
  printf("argv[1]: Press the Key Enter to Exit...\n");
  getchar();
  ...
  _TCHAR less=0,two=0,op=0,greater1=0,greater2=0;//greater22=0
  int res = swscanf(argv[1],L"%[a-zA-Z0-9]%c%[a-zA-Z0-9] %c%c%[a-zA-Z0-9]%c%[a-zA-Z0-9]%c%[a-zA-Z0-9]",
    pr0,&less,streamIn,&two,&greater1,streamErr,&op,pr1);
  
  /*_TCHAR less = 0, two = 0, op = 0, greater1 = 0, greater2 = 0, greater22=0;
  int res = swscanf(argv[1], L"%[a-zA-Z0-9]%c%[a-zA-Z0-9] %c%c%[a-zA-Z0-9]%c%[a-zA-Z0-9]%c%c%[a-zA-Z0-9]",
    pr0, &less, streamIn, &two, &greater1, streamErr, &op, pr1, &greater2, &greater22, streamOut);
  */
  if(res == EOF)
  {
  ...
  else if(res != 8)//if(res != 11)
  {
    printf("Error: res != 10(11), the number of fields successfully converted and assigned;res:%d\n",res);
    printf("Press the Key Enter to Exit...\n");
    getchar();
    return 0;
  ...
  if(less!='<'||two!='2'||greater1!='>')//|| greater22 != '>')
  {
  ...
  wprintf(L"%s%c%s %c%c%s%c%s\n",pr0,less,streamIn,two,greater1,streamErr,op,pr1);
  
  //wprintf(L"%s%c%s %c%c%s%c%s%c%c%s\n", pr0, less, streamIn, two, greater1, streamErr, op, pr1, greater2, greater22, streamOut);
  ...
  //printf("Press the Key Enter to Exit...\n");
  printf("Press the Key Enter to be continued...\n");
    getchar();
    //return 0;
  ...
  //  &saFileOut,
  //  CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,NULL);//CREATE_ALWAYS OPEN_EXISTING OPEN_ALWAYS
  ...
  /*if (hFileOut == INVALID_HANDLE_VALUE)
    ...
    //SetFilePointer(hFileOut, 0, 0, 2);
    //
  STARTUPINFO startInfoProcess0, startInfoProcess1;

  //1)For Pipe
  DWORD nsize = 100;
  BOOL bPipe = CreatePipe(&hReadPipe, &hWritePipe, &saPipe, nsize);//0
  if (!bPipe)
  {
  ...
  startInfoProcess1.hStdOutput = GetStdHandle(STD_OUTPUT_HANDLE);
