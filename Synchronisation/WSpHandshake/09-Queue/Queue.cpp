...
BOOL CQueue::Append(PELEMENT pElement, DWORD dwTimeout) {
    ...
    if (dw == WAIT_OBJECT_0) {
        ...
        if (fOk) {
            ...
            pElement->queueTime = GetTickCount();
...
DWORD WINAPI ServerThread(PVOID pvParam) {
    ...
    while (!g_fShutdown) {
        ...
        if (g_q.Remove(&e, 5000)) {
            ...
            DWORD time = GetTickCount() - e.queueTime;
            wsprintf(sz, TEXT("%d: Processing %d:%d, time: %u ms"),
                nThreadNum, e.m_nThreadNum, e.m_nRequestNum, time);
            ...
            wsprintf(sz, TEXT("%d: (you missed attemped)"), nThreadNum);
        }
