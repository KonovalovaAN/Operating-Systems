...
DWORD WINAPI ServerThread(PVOID pvParam) {
    HMODULE hLibrary = LoadLibraryW(L"Konovalova.dll");
    if (!hLibrary) {
        return 1;
    }

    typedef float (*AddFunction)(float, float);
    typedef float (*MultiplyFunction)(float, float);

    AddFunction add = (AddFunction)GetProcAddress(hLibrary, "add");
    MultiplyFunction multiply = (MultiplyFunction)GetProcAddress(hLibrary, "multiply");
    ...
    while (!fShutdown) {
        ...
        if (!fShutdown) {
            ...
            if (_stscanf_s(g_szSharedRequestAndResultBuffer, TEXT("%9s %f %f"), operation, (unsigned)_countof(operation), &x, &y) == 3) {
                float result = 0.0f;

                if (_tcsicmp(operation, TEXT("add")) == 0) {
                    result = add(x, y);
                }
                else if (_tcsicmp(operation, TEXT("mult")) == 0) {
                    result = multiply(x, y);
                }

                _stprintf_s(g_szSharedRequestAndResultBuffer, TEXT("%f"), result);
            }
        }
